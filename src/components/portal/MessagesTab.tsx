...
export function MessagesTab({
  ...
  async function send() {
    ...
    const { data, error } = await supabase
      ...
      .insert({ client_id: clientId, sender: 'client', author_name: clientName, body })
      .select('*')
      .single()
    ...
    <div className="card-panel gold-border rounded-2xl flex flex-col h-[600px]">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-5">
        {items.length === 0 ? (
          ...
          items.map((m) => (
            <div key={m.id} className={`flex gap-3 ${m.sender === 'client' ? 'flex-row-reverse text-right' : ''}`}>
              ...
              <div
                className={`max-w-md rounded-2xl px-4 py-3 text-sm ${
                  m.sender === 'client' ? 'bg-gold/15 text-white' : 'bg-white/5 text-white/80'
                }`}
              >
