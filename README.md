...
Unread-message badges (on the admin roster, admin Messages tab, and portal Messages tab) work the same way, tracked via `client_messages.read_at`. Admins already have full read/write access to that table via RLS, so the admin side marks messages read with a direct update from the browser. Clients don't have (and shouldn't have) a blanket update policy on that table — a client could otherwise rewrite any message in their own thread, not just flip a read flag — so the portal side marks messages read via `POST /api/portal/messages/mark-read`, which uses the service role scoped to exactly the caller's own admin-authored, unread rows.
...
- **Messages** — the same `client_messages` thread the admin portal's Messages tab writes to, so messages sent by either side show up for both, in real time on next load. The **Messages** nav item shows a live unread-count badge for anything the care team sent that the client hasn't opened yet; opening the tab marks those messages read.
